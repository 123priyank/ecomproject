...
@section('content')
                            ...
                            <ul class="aa-product-catg">
                                ...
                                @if(isset($product[0]))
                                    @foreach($product as $productArr)
                                        ...
                                        <li>
                                            <figure>
                                                <a class="aa-product-img" href="{{url('front/product/'.$productArr->slug)}}"><img src="{{asset('main_image/'.$productArr->image)}}" alt="{{$productArr->name}}"></a>
                                                ...
                                                <figcaption>
                                                    <h4 class="aa-product-title"><a href="{{url('front/product/'.$productArr->slug)}}">{{$productArr->name}}</a></h4>
                        ...
                        <div class="aa-sidebar-widget">
                            ...
                            <ul class="aa-catg-nav">
                                @foreach($categories_list as $cat_list)
                                    @if($slug==$cat_list->category_slug)
                                        <li><a href="{{url('front/category/'.$cat_list->category_slug)}}" class="left_cat_active">{{$cat_list->category_name}}</a></li>
                                    @else
                                        <li><a href="{{url('front/category/'.$cat_list->category_slug)}}">{{$cat_list->category_name}}</a></li>
                                    @endif
                                @endforeach
                            </ul>
                        </div>
    ...
    <form id="categoryFilter">
        ...
        <input type="hidden" id="color_filter" name="color_filter" value=""/>
    </form>
